...
data.nano$QUAL_MIN <- sapply(data.nano$ALT_QUALITY, function(x) min(as.numeric(strsplit(x, ",")[[1]])))
data.nano <- data.nano[data.nano$DELETION_NUM == 0 & data.nano$BP_RATIO > 5 & data.nano$QUAL_MIN >= 10, ]
...
rand_sample_num <- round(nrow(data.nano)/(10**(floor(log10(nrow(data.nano))))), digits=1) * 10**(floor(log10(nrow(data.nano)))-1)
print(paste(c('Random sample: ', rand_sample_num), collapse=''))

#parameters <- c('SKIP_NUM', 'PUZZLE_NUM', 'HOMOPOLYMER', 'SEQ_ERROR_P', 'QUAL_MEAN', 'ALT_NUM')
parameters <- c('SKIP_NUM', 'PUZZLE_NUM', 'HOMOPOLYMER', 'QUAL_MEAN', 'ALT_NUM')
QUAL_MEAN_iter <- c(); HOMOPOLYMER_iter <- c(); PUZZLE_NUM_iter <- c(); ALT_NUM_iter <- c(); SKIP_NUM_iter <- c() # SEQ_ERROR_P_iter <- c(); 
final_parameters <- c()
for (i in seq(1,100)){
    ...
    glm_test <- glm(GS~DELETION_NUM+SKIP_NUM+BP_RATIO+PUZZLE_NUM+HOMOPOLYMER+QUAL_MEAN+ALT_NUM, data=rand_test, family=binomial(link='logit'))
...
parameter.df <- data.frame(ITER=seq(1, length(QUAL_MEAN_iter)), QUAL_MEAN=QUAL_MEAN_iter, HOMOPOLYMER=HOMOPOLYMER_iter, PUZZLE_NUM=PUZZLE_NUM_iter, ALT_NUM=ALT_NUM_iter, SKIP_NUM=SKIP_NUM_iter)#, SEQ_ERROR_P=SEQ_ERROR_P_iter)
final_parameter <- c()
for (Feature in colnames(parameter.df)[2:ncol(parameter.df)]){
...
print(paste(c('final_parameters: ', paste(final_parameters, collapse=', '), collapse='')))
...
print(output.type)
write.table(output.type, paste(c(outPrefix, '.filtered.edit_type.txt'), collapse=''), sep='\t',row.names=F, quote=F)

write.table(data.nano.filt, paste(c(outPrefix, '.filtered.txt'), collapse=''), sep='\t', row.names=F, quote=F)
