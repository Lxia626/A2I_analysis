...
cd ../
## ATTENTION: after the script has finished, whether STAR INDEX had finished should be checked before SJM the job

# Basic options
home_wd=`pwd`
FASTQ_DIR=${home_wd}/FASTQ/trim_reads # need to specify accordingly
...
SraFile=${home_wd}/FASTQ/SraAccList.csv # to specify if needed
SAMPLEs=($(cat ${SraFile}))
...
for SAMPLE in ${SAMPLEs[@]}; do
    ...
    x=`expr ${x} + 1`
    echo -e "order ${sample_name}_1_GATKCallEdits before ${SAMPLE}_01_CHANGE" >> ${UNIQUE_ID}.order # ${SAMPLE_list[x]}_First
    echo -e "order ${sample_name}_2_GATKCallEdits before ${SAMPLE}_01_CHANGE" >> ${UNIQUE_ID}.order
    if [[ ${x} -lt ${#SAMPLEs[@]} ]]; then
        echo -e "order ${SAMPLE}_01_CHANGE before ${SAMPLEs[x]}_1_First" >> ${UNIQUE_ID}.order
        echo -e "order ${SAMPLE}_01_CHANGE before ${SAMPLEs[x]}_2_First" >> ${UNIQUE_ID}.order
    else
        echo -e "order ${SAMPLE}_01_CHANGE before site01_CHANGE" >> ${UNIQUE_ID}.order
    fi
...
x=0
echo -e "order site01_CHANGE before ${SAMPLEs[0]}_MERGE" >> ${UNIQUE_ID}.order
for SAMPLE in ${SAMPLEs[@]}; do
    sample_name=${SAMPLEs[x]}
    grep 'order' ${SAMPLE}/${sample_name}.bam.sjm >> ${UNIQUE_ID}.order
    x=`expr ${x} + 1`
    if [[ ${Alu} == "Alu" || ${Alu} == "nonAlu" ]]; then
        echo -e "order ${sample_name}_CAT before ${SAMPLE}_02_CHANGE" >> ${UNIQUE_ID}.order
    else
        echo -e "order ${sample_name}_CAT_NoAlu before ${SAMPLE}_02_CHANGE" >> ${UNIQUE_ID}.order
    fi
    if [[ ${x} -lt ${#SAMPLEs[@]} ]]; then
        echo -e "order ${SAMPLE}_02_CHANGE before ${SAMPLEs[x]}_MERGE" >> ${UNIQUE_ID}.order
    fi
    grep -vE 'order|log_dir' ${SAMPLE}/${sample_name}.bam.sjm >> ${UNIQUE_ID}.cmd
    echo -e "job_begin\n name ${SAMPLE}_02_CHANGE\n memory 3G\n directory `pwd`\n cmd_begin" >> ${UNIQUE_ID}.cmd
    echo -e "/usr/bin/date;" >> ${UNIQUE_ID}.cmd
    echo -e "cmd_end\njob_end\n" >> ${UNIQUE_ID}.cmd
done
